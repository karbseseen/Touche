...
def callback(user_id: int, game: Game, event: field.Event):
	...
	if event.card_index == 5:
		if used_cell and used_cell.user_id == user_id and used_cell.type == UsedType.Normal:
			...
			figure_success = player.figures.add_cell(event.cell_index)
			if figure_success:
				for cell_index in figure_success.cell_indices:
					game.cell_by_index[cell_index].type = UsedType.Final
				game.cell_history.append(FinalFigure(figure_success.cell_indices, user_id))
		...
		if basic_valid() or free_valid() or joker_valid():
			...
			game.updated_cell = event.cell_index
			game.lead = game.lead.opponent
...
def _move_to_undo_index(user_id: int, game: Game):
	...
	if game.get_player(user_id).figures.cell_num: return _SemiFigure
